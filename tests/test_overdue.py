...
def test_overdue_graduated_appear_and_priority_by_due_ts(app_client, fixed_now, monkeypatch):
    ...
    # Cria dois cards
    A = _mk_card(c, deck_id=deck["public_id"], front="A?", back="a")
    B = _mk_card(c, deck_id=deck["public_id"], front="B?", back="b")

    # Gradua ambos rapidamente (EASY → sai do learning e vira revisão diária)
    r = c.post(f"/cards/{A['public_id']}/review", json={"button": "easy"})
    assert r.status_code == 200, r.text
    r = c.post(f"/cards/{B['public_id']}/review", json={"button": "easy"})
    assert r.status_code == 200, r.text
    ...
    with sqlite3.connect(api.DB) as db:
        ...
        db.execute(
            "UPDATE cards SET due=?, due_ts=? WHERE public_id=?",
            (dueA_day, dueA_ts, A["public_id"]),
        )
    ...
    assert r1.json()["public_id"] == A["public_id"]
    ...
    r3 = c.get("/reviews/next", params={"deck_id": deck["public_id"]})
    assert r3.status_code == 200, r3.text
    assert r3.json()["public_id"] == B["public_id"]
