...
def _get_card(client, card_id):
    r = client.get(f"/cards/{card_id}")
    assert r.status_code == 200, r.text
    return r.json()

def _peek_next(client, deck="geo", ok=True):
    r = client.get(f"/reviews/next?deck={deck}")
    if ok:
        assert r.status_code == 200, r.text
        return r.json()
    return r
...
def test_prioridade_reviews_next_devido_real_vem_primeiro(app_client, fixed_now):
    ...
    cardA = _create_card(c, deck="geo", front="A?", back="a")
    # Card B (learning): GOOD 1ª vez → 10m
    cardB = _create_card(c, deck="geo", front="B?", back="b")

    # B: GOOD para ir para 10m
    r = c.post(f"/cards/{cardB['id']}/review", json={"button": "good"})
    assert r.status_code == 200, r.text
